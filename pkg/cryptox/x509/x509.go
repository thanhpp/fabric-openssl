...
func namedCurveFromOID(oid asn1.ObjectIdentifier) elliptic.Curve {
	...
	case oid.Equal(bcy256.OIDNamedCurves):
		return bcy256.Curve
	}
...
func oidFromNamedCurve(curve elliptic.Curve) (asn1.ObjectIdentifier, bool) {
	...
	case bcy256.CurveName:
		return bcy256.OIDNamedCurves, true
	}
...
func (c *Certificate) ToStd() *stdx509.Certificate {
	return &stdx509.Certificate{
		...
		SignatureAlgorithm:          stdx509.SignatureAlgorithm(c.SignatureAlgorithm),
		PublicKeyAlgorithm:          stdx509.PublicKeyAlgorithm(c.PublicKeyAlgorithm),
		...
		KeyUsage:                    stdx509.KeyUsage(c.KeyUsage),
		Extensions:                  c.Extensions,
		ExtraExtensions:             c.ExtraExtensions,
		UnhandledCriticalExtensions: c.UnhandledCriticalExtensions,
		ExtKeyUsage: sliceMap(c.ExtKeyUsage, func(a ExtKeyUsage) stdx509.ExtKeyUsage {
			return stdx509.ExtKeyUsage(a)
		}),
...
func signingParamsForPublicKey(pub any, requestedSigAlgo SignatureAlgorithm) (hashFunc crypto.Hash, sigAlgo pkix.AlgorithmIdentifier, err error) {
	...
	case *ecdsa.PublicKey:
		...
		switch pub.Curve {
		case elliptic.P224(), elliptic.P256(), bcy256.Curve:
			hashFunc = crypto.SHA256
...
func ConvertCert(c *stdx509.Certificate) *Certificate {
	return &Certificate{
		Raw:                         c.Raw,
		RawTBSCertificate:           c.RawTBSCertificate,
		RawSubjectPublicKeyInfo:     c.RawSubjectPublicKeyInfo,
		...
		RawIssuer:                   c.RawIssuer,
		Signature:                   c.Signature,
		SignatureAlgorithm:          SignatureAlgorithm(c.SignatureAlgorithm),
		PublicKeyAlgorithm:          PublicKeyAlgorithm(c.PublicKeyAlgorithm),
		PublicKey:                   c.PublicKey,
		Version:                     c.Version,
		SerialNumber:                c.SerialNumber,
		Issuer:                      c.Issuer,
		...
		ExtraExtensions:             c.ExtraExtensions,
		UnhandledCriticalExtensions: c.UnhandledCriticalExtensions,
		ExtKeyUsage: sliceMap(c.ExtKeyUsage, func(a stdx509.ExtKeyUsage) ExtKeyUsage {
			return ExtKeyUsage(a)
		}),
		UnknownExtKeyUsage:          c.UnknownExtKeyUsage,
		BasicConstraintsValid:       c.BasicConstraintsValid,
		IsCA:                        c.IsCA,
...
func sliceMap[A, B any](s []A, fn func(A) B) []B {
	out := make([]B, 0, len(s))
	for i := range s {
		out = append(out, fn(s[i]))
	}
	return out
}
