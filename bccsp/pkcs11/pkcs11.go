...
func (csp *Provider) KeyGen(opts bccsp.KeyGenOpts) (k bccsp.Key, err error) {
	...
	case *bccsp.ECDSAKeyGenOpts:
		...
		oPub, err := opensslw.ConvertECDSAPublicKey(pub)
		if err != nil {
...
func (csp *Provider) GetKey(ski []byte) (bccsp.Key, error) {
	...
	oPub, err := opensslw.ConvertECDSAPublicKey(pubKey)
	if err != nil {
		return nil, errors.Wrapf(err, "convert openssl ecdsa key")
	}
...
func (csp *Provider) signECDSA(k ecdsaPrivateKey, digest []byte) ([]byte, error) {
	...
	s, err = opensslw.ECDSAToLowS(k.pub.pub, s)
	if err != nil {
		return nil, err
	}
...
func (csp *Provider) verifyECDSA(k ecdsaPublicKey, signature, digest []byte) (bool, error) {
	...
	lowS, err := opensslw.ECDSAIsLowS(k.pub, s)
	if err != nil {
		return false, err
	}
	if !lowS {
